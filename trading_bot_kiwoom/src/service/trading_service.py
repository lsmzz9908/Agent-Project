...
class TradingService:
    def __init__(
        ...
        if getattr(self.notifier, "logger", None) is None:
    ...
    def _ensure_live_stack(self):
        ...
        if self.api is None:
            ...
            self.api.register_callback("chejan", self.om.handle_chejan)
            self.api.register_callback("chejan", self._on_chejan_stats)

    def _on_chejan_stats(self, payload: dict):
    ...
    def is_logged_in(self) -> bool:
        return bool(self.broker and self.broker.account_no)
    ...
    def start_swing_batch(self, universe: list[str], live: bool = True) -> RunSummary:
        ...
        if live and not self.is_logged_in():
            self._login()
    ...
    def smoke_notify(self):
        self.notifier.send_test("📢 알림 테스트")
